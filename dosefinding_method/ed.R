############################################################################################################################
#### ED (without Micallef (2018) EWOC decision rule) - Estimation of the toxicity (Dose-exposure-toxicity relationship) ####
############################################################################################################################

# Core function
ed_model <- function(num, doses, ref_dose, x, y, conci, auc_s, n_sampling_timepoints, time_sampling,
                     real_sampling, PK_parameters, targeted_tox, proba_safety = 0.9, options, seed) {
  
  ### Setting up the appropriate parameters to instantiate the alternative ED method
  # Initilization phase with BLRM 
  alpha_prior <- c(f_logit(targeted_tox), 0.4)
  beta_prior <- c(0.6, 1)
  # Main model using ED specific methodology
  beta2_mean <- -6
  beta2_sd <- 5
  log_beta3_mean <- 2
  log_beta3_sd <- 1
  ...
  if(num <= N/2) {
    ...
    log_alpha_credible <- as.vector(quantile(rstan::extract(reg, pars = "log_alpha")$log_alpha, probs = c(0.025, 0.975)))
    log_beta_credible <- as.vector(quantile(rstan::extract(reg, pars = "log_beta")$log_beta, probs = c(0.025, 0.975)))
    p_estim_dose_2.5pct <- f_inv_logit(log_alpha_credible[1] + exp(log_beta_credible[1])*log(doses/ref_dose))
    p_estim_dose_97.5pct <- f_inv_logit(log_alpha_credible[2] + exp(log_beta_credible[2])*log(doses/ref_dose))
    credible_interval_per_dose <- matrix(sapply(1:length(doses), function(m) c(p_estim_dose_2.5pct[m], p_estim_dose_97.5pct[m])), 
                                         nrow = 2,
                                         ncol = length(doses), 
    ...
    ### Running the main design for the ED method once half of the patients have been included using an initialization phase with a BLRM
    ka <- PK_parameters[1]
    ...
    saemix.model <- saemixModel(model = model_1cpt, description = "One-compartment model with first-order absorption",
                                psi0 = matrix(c(ka*(1+random_lowU_initialization[1]), V_pop*(1+random_lowU_initialization[2]), Cl_pop*(1+random_lowU_initialization[2])), ncol = 3, byrow = TRUE, dimnames = list(NULL, c("ka", "V", "Cl"))),
